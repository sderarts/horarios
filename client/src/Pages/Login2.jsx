    ...
    useEffect(() => {
        ...
        const checkUserStatus = async () => {
            if (user) {
                setIsLoading(true);
                try {
                    // Llamada al backend para verificar si el usuario ya está registrado
                    const response = await fetch(`http://localhost:8800/auth/checkUser/${user.uid}`);
                    const data = await response.json();

                    if (data.exists) {
                        if (data.rol === 1) {
                            navigate('/'); // Redirige al perfil académico si ya es académico
                        } else {
                            navigate('/'); // Redirige al perfil de alumno o donde desees
                        }
                    } else {
                        navigate('/register'); // Si no existe, redirige a la página de registro
                    }
                } catch (error) {
    ...
    return (
        <div>
            {isLoading ? (
                ...
                !user ? (
                    <Login /> // Si no hay usuario, muestra el formulario de login
                ) : (
